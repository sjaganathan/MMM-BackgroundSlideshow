...
<table width="100%">
	...
	<tbody>
		...
		<tr>
			...
			<td>String value, a list of image properties to display in the image info div, separated by commas.  Possible values are : date (EXIF date from image), short-date (concise EXIF date), name (image name), tags (EXIF/IPMC tags).
			For the iamge name, the relative path from that defined in imagePaths is displayed if the recursiveSubDirectories option is set to true.<br>
				<br><b>Example:</b> <code>date,short-date,name,tags</code>
				<br><b>Default value:</b> <code>name</code>
				<br>This value is <b>OPTIONAL</b>
			</td>
		</tr>
